	...
	scanner.scan({max:50, vendor:false,timeout:1}, (err, arr) => {
		for(var ipAddr of arr){
			if(ipAddr.mac == rationalizedMAC){
				...
				logger.info("Found Yamaha target with IP ("+ipAddr.ip+") matching MAC: "+rationalizedMAC);
				retVal = {status:200, msg: "OK"};
				break;
			};
		};
		if(retVal.status == 303){
			logger.info("Searched local network but did not find a MAC which matched, using localhost as target");
			// TODO : start localhost server receiver-mockResponse as subprocess so responses can be tested at least.
		}
	});
};
...
exports.getPowerStatus = async function(){
	return new Promise(async function(resolve, reject){
		try{
			var basicInfo = await receiver.getBasicInfo();
			if(basicInfo.status == 200){
				...
				var powerObj = {
					power : basicInfoJSON.YAMAHA_AV.Main_Zone.Basic_Status.Power_Control.Power._text
				};

				logger.debug("Got power status OK: "+powerObj.power)
				resolve({status:200,msg:"OK",powerObj});
			} else {
				var retVal = "getBasicInfo status code not OK: "+basicInfo.status;
				logger.error(retVal);
				reject({status:500,msg:retVal})
			...
			var errMsg = "Error - receiver.getPowerStatus():: "+err;
			logger.error(errMsg);
			reject({status:500,msg:errMsg});
		}		
	});	
